...
def Periodebestimmen(DATEN, messreihe):
    ...
    def f(t, A, B, T, phi, y_0):
        return A * np.exp(-B * t) * np.cos(2* np.pi / T * t + phi) + y_0
...
def auswertung_messreihe(DATEN, i, rausch_sigma=None, fit_trim=[0,-1], plot_intervall=[3000,4000]):
    ...
    Terr = np.sqrt(pcov[2,2])
    
    A_fit, B_fit, T_fit, phi_fit, y_0_fit = popt
    w_fit = 2 * np.pi / T_fit
...
# g aus gemitteltem T
g_mean = (4 * np.pi**2 / T_mean**2 * (l_p.n / 1000) * (1 + (1/8) * ((d_p.n / 1000) / (l_p.n / 1000))**2))
...
g_l_d = (4 * np.pi**2 / T_mean**2 * (l_p / 1000) * (1 + (1/8) * ((d_p/1000) / (l_p/1000))**2))
