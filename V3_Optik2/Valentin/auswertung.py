...
stat_a_p = std_a_p / len(a_p_list)

mean_b_p = np.mean(b_p_list)
std_b_p  = np.std(b_p_list, ddof=1)
stat_b_p = std_b_p / len(b_p_list)

mean_chiq_dof = np.mean(chiq_dof_p)
...
stat_a_k = std_a_k / len(a_k_list)

mean_b_k = np.mean(b_k_list)
std_b_k  = np.std(b_k_list, ddof=1)
stat_b_k = std_b_k / len(b_k_list)

mean_chiq_dof_k = np.mean(chiq_dof_k)
...
stat_a_l = std_a_l / len(a_l_list)

mean_b_l = np.mean(b_l_list)
std_b_l  = np.std(b_l_list, ddof=1)
stat_b_l = std_b_l / len(b_l_list)

mean_chiq_dof_l = np.mean(chiq_dof_l)
